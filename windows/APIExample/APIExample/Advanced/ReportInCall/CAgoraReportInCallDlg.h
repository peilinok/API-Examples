...
public:
	...
	*/
	 ...
	 */
		...
		if (m_hMsgHanlder)
            ::PostMessage(m_hMsgHanlder, WM_MSGID(EID_LOCAL_VIDEO_STATS),
                          (WPARAM) new LocalVideoStats(stats), (LPARAM)source);
	}
	 ...
	 */
        virtual void onLocalVideoStateChanged(
            VIDEO_SOURCE_TYPE source, LOCAL_VIDEO_STREAM_STATE localVideoState,
